...
public class Event {
    ...
    private void checkState(EventState newState) {
        if (newState == PUBLISHED && state != PENDING) {
            throw new IncorrectActionException("Опубликовать можно только ожидающие события!");
        }
        if (newState == CANCELED && state == PUBLISHED) {
            throw new IncorrectActionException("Отклонить можно только неопубликованное событие!");
        }
        if (newState == PENDING && state == PUBLISHED) {
            throw new IncorrectActionException("Изменить можно только неопубликованное событие!");
        }
    }
}
